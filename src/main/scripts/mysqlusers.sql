...
create user if not exists `bookadmin`@`%` identified with mysql_native_password by 'root';
...
create user if not exists `bookuser`@`%` identified with mysql_native_password by 'root';
grant select, trigger on `bookdb`.* to `bookuser`@`%`;
flush privileges;